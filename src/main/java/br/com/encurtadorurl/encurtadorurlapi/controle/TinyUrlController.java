...
import br.com.encurtadorurl.encurtadorurlapi.vo.UrlDto;
...
public class TinyUrlController {
    ...
    public ResponseEntity create(@RequestBody final String url) {
      ...
      /*  if (!urlValidator.isValid(url)) {
            ...
            return ResponseEntity.badRequest().body(new RuntimeException("Invalid URL."));
        }*/
    ...
    public ResponseEntity getUrl(@PathVariable final String id) {
        ...
        if (Objects.isNull(urlDto)) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new RuntimeException("No such key exists."));
        } else {
